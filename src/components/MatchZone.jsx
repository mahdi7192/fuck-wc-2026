...
const selectStarting11 = (squad) => {
  ...
  selectedDefs.forEach((p, idx) => {
    lineup.push({ ...p, position: 'DEFENDER', shirtNumber: p.shirtNumber || (idx + 2) });
  });
  
  // Select MID (up to 4)
  const selectedMids = midfielders.slice(0, 4);
  selectedMids.forEach((p, idx) => {
    lineup.push({ ...p, position: 'MIDFIELDER', shirtNumber: p.shirtNumber || (idx + 6) });
  });
  
  // Select FWD (up to 2)
  const selectedFwds = forwards.slice(0, 2);
  selectedFwds.forEach((p, idx) => {
    lineup.push({ ...p, position: 'FORWARD', shirtNumber: p.shirtNumber || (idx + 10) });
  });
...
export default function MatchZone({ matchId, onBack }) {
  const [authToken, setAuthToken] = useState(import.meta.env.PUBLIC_FOOTBALL_API_TOKEN || 'ef19c292505a42a8acb1fe4c95ef98f3');
  const [matchStatus, setMatchStatus] = useState('WAITING'); // WAITING, LIVE, FINISHED
  
  // Teams State
  const [homeTeam, setHomeTeam] = useState({ name: 'در حال بارگذاری...', crest: '' });
  const [awayTeam, setAwayTeam] = useState({ name: 'در حال بارگذاری...', crest: '' });
  ...
  const allPlayers = [...homePlayers, ...awayPlayers, ...homeBench, ...awayBench];
  const maxRants = allPlayers.reduce((max, p) => (p.totalRants > max ? p.totalRants : max), 0);
  ...
  useEffect(() => {
    if (selectedPlayer) {
      ...
      if (match && (match.photoUrl !== selectedPlayer.photoUrl || match.totalRants !== selectedPlayer.totalRants)) {
  ...
  const getPersianPositionAbbr = (pos) => {
    switch (pos?.toUpperCase()) {
      case 'GOALKEEPER': return 'دروازه‌بان';
      case 'DEFENDER': return 'مدافع';
      case 'MIDFIELDER': return 'هافبک';
      case 'FORWARD': return 'مهاجم';
      default: return 'بازیکن';
    }
  };

  const getPositionColor = (pos) => {
    switch (pos?.toUpperCase()) {
      case 'GOALKEEPER': return 'var(--color-gk)';
      case 'DEFENDER': return 'var(--color-def)';
      case 'MIDFIELDER': return 'var(--color-mid)';
      case 'FORWARD': return 'var(--color-fwd)';
  ...
  const simulateRoster = (starters, bench, side, homeTeamName, awayTeamName, matchScore, matchMinutes, status) => {
    ...
    const updatedStarters = starters.map((p, idx) => {
      ...
      if (isLiveOrFinished) {
        if (idx === 8 && matchMinutes >= 60 && updatedBench.some(b => b.subbedIn === 60)) {
          subbedOutMin = 60;
        } else if (idx === 9 && matchMinutes >= 72 && updatedBench.some(b => b.subbedIn === 72)) {
          subbedOutMin = 72;
        } else if (idx === 9 && matchMinutes >= 75 && updatedBench.some(b => b.subbedIn === 75)) {
          subbedOutMin = 75;
        } else if (idx === 10 && matchMinutes >= 82 && updatedBench.some(b => b.subbedIn === 82)) {
          subbedOutMin = 82;
        }
      }
      ...
      const minutesPlayed = subbedOutMin 
        ? subbedOutMin 
        : (isLiveOrFinished ? matchMinutes : 0);
    ...
    const isGermanyMatch = (homeTeamName === 'Germany' && awayTeamName === 'Curaçao') || 
                           (homeTeamName === 'Curaçao' && awayTeamName === 'Germany');
    
    const allPlayers = [...updatedStarters, ...updatedBench].filter(p => p.minutesPlayed > 0);

    if (isLiveOrFinished && teamScore > 0) {
      if (isGermanyMatch) {
        const isGermanyTeam = (isHome && homeTeamName === 'Germany') || (!isHome && awayTeamName === 'Germany');
        if (isGermanyTeam) {
          let goalsAssigned = 0;
          const targetScorers = [
            { name: 'Havertz', max: 2 },
          ...
          for (const target of targetScorers) {
            ...
            if (found) {
              const goalsToAssign = Math.min(target.max, teamScore - goalsAssigned);
          ...
          if (remainingGoals > 0) {
            const potentialScorers = allPlayers.filter(p => p.position === 'FORWARD' || p.position === 'MIDFIELDER');
          ...
          if (locadia) {
            ...
            const forward = allPlayers.find(p => p.position === 'FORWARD') || allPlayers[0];
    ...
    if (isLiveOrFinished && teamScore > 0) {
      ...
      const potentialAssisters = allPlayers.filter(p => p.position !== 'GOALKEEPER');
      let assisterIdx = 0;
      goalsList.forEach((goalPlayerId, idx) => {
        if (idx % 3 !== 2 && potentialAssisters.length > 1) {
          const assister = potentialAssisters.find(p => p.id !== goalPlayerId) || potentialAssisters[assisterIdx % potentialAssisters.length];
          if (assister) {
            assister.assists += 1;
            assisterIdx++;
          }
    ...
    allPlayers.forEach(p => {
      ...
      const nameSeed = p.name.charCodeAt(0) + p.name.charCodeAt(p.name.length - 1);
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.matchId === matchId) {
          ...
          console.log(`[Client] SSE update received for player ${playerId}: ${totalRants} rants (key: ${rantKey})`);
          ...
          const updateRants = (prevList) => {
            return prevList.map(p => {
              if (p.id === playerId) {
                ...
                if (p.baseRating) {
                  updated.rating = Math.max(1.0, parseFloat((p.baseRating - totalRants * 0.2).toFixed(1)));
                }
                return updated;
              }
              return p;
  ...
  const handleFetchRealMatch = async () => {
    ...
    try {
      ...
      console.log(`[Client] Fetching match details from: ${fetchUrl} (ID: ${matchId})`);
      ...
      if (!res.ok) {
        ...
        throw new Error(`خطای سرور (${res.status}): ${errorData.message || 'پاسخی دریافت نشد'}`);
      }

      const selectedMatch = await res.json();
      console.log(`[Client] Parsed response data:`, selectedMatch);

      if (!selectedMatch || !selectedMatch.homeTeam || !selectedMatch.awayTeam || !selectedMatch.score) {
        console.warn(`[Client] Validation failed for match data structure. SelectedMatch key check:`, {
          hasMatch: !!selectedMatch,
          hasHomeTeam: !!selectedMatch?.homeTeam,
          hasAwayTeam: !!selectedMatch?.awayTeam,
          hasScore: !!selectedMatch?.score
        });
        throw new Error("اطلاعات بازی نامعتبر است یا در حال حاضر در دسترس نیست");
      }
      ...
      const elapsedMin = isLive ? (selectedMatch.elapsed || 1) : (isFinished ? 90 : 0);
      ...
      const homeStarters = simulatedHome.starters.map(p => ({
        ...p,
        rating: p.baseRating ? Math.max(1.0, parseFloat((p.baseRating - (p.totalRants || 0) * 0.2).toFixed(1))) : p.baseRating
      }));
      ...
      const simulatedHome = simulateRoster(fallbackHome, [], 'home', 'Home', 'Away', { home: 0, away: 0 }, 0, 'WAITING');
      const simulatedAway = simulateRoster(fallbackAway, [], 'away', 'Home', 'Away', { home: 0, away: 0 }, 0, 'WAITING');
  ...
  const handleRant = (playerId, rantKey, event) => {
    ...
    const allPlayersList = [...homePlayers, ...awayPlayers, ...homeBench, ...awayBench];
    ...
    const isInStarting = homePlayers.some(p => p.id === playerId) || awayPlayers.some(p => p.id === playerId);
    const setList = isInStarting 
      ? (isHome ? setHomePlayers : setAwayPlayers)
      : (isHome ? setHomeBench : setAwayBench);

    const rantPersian = PREDEFINED_RANTS.find(r => r.key === rantKey)?.persianText || '';

    setList(prev => prev.map(p => {
      if (p.id === playerId) {
        const newRants = { ...p.rants, [rantKey]: (p.rants[rantKey] || 0) + 1 };
        ...
        if (p.baseRating) {
          updated.rating = Math.max(1.0, parseFloat((p.baseRating - newTotal * 0.2).toFixed(1)));
        }
  ...
  const getRantKingSummary = (king) => {
    ...
    const topRantText = PREDEFINED_RANTS.find(r => r.key === topRantKey)?.persianText || 'راه برو فقط!';
  ...
  const renderPlayerRow = (player) => {
    ...
    return (
      <div
        key={player.id}
        onClick={isWaiting ? undefined : () => {
          setSelectedPlayer(player);
          setIsModalOpen(true);
        }}
        className={`native-player-row ${isHome ? 'home-row' : 'away-row'}`}
        style={{
          opacity: (player.subbedIn === null && player.minutesPlayed === 0) ? 0.65 : 1,
      ...
      >
        <div className="player-row-right">
          <div className="native-player-avatar-container">
            <img
              src={player.photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(player.name)}&backgroundColor=${isHome ? '00e676' : 'ff3e3e'}`}
              alt={player.name}
          ...
          <div className="native-player-name-wrapper" style={{ height: '36px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            <span className="native-player-name" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              {lastName}
              <span 
                className="native-player-rants-count-inline"
                style={{ fontSize: '0.65rem', color: player.totalRants > 0 ? 'var(--color-danger)' : 'var(--text-hint)', marginRight: '4px', fontWeight: '500' }}
              >
                ({player.totalRants})
              </span>
              {hasMostRants && (
            ...
            <span className={`active-rant-bubble ${player.activeRant ? 'active' : ''}`}>
              {player.activeRant || '\u00a0'}
            </span>
          </div>
        </div>
        ...
        <div className="player-row-left">
          {player.goals > 0 && (
            <span className="player-badge-stat goals" title={`${player.goals} Goals`}>
  ...
  const renderTeamRoster = (starters, bench) => {
    return (
      <div className="native-roster-list">
        ...
        {bench.length > 0 && (
          <>
            <h3 className="native-section-title" style={{ marginTop: '16px' }}>بازیکنان ذخیره</h3>
            <div className="native-players-group">
  ...
  if (loading && !homePlayers.length) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '300px', gap: '12px' }}>
        <div className="blink" style={{ fontSize: '1rem', fontWeight: '900', color: 'var(--md-sys-color-on-background)' }}>
  ...
  return (
    <div className="match-zone-container">
      ...
      {apiError && (
        <div className="settings-error" style={{ margin: '8px auto', width: '100%', textAlign: 'center' }}>
          ⚠️ {apiError}
        </div>
      )}
        ...
        <div className="native-scoreboard">
          <div className="native-scoreboard-row">
            ...
            <div className="native-scoreboard-team home">
              {renderCrest(homeTeam.crest || '🇮🇷', homeTeam.name, 'native-scoreboard-crest')}
            ...
            <div className="native-scoreboard-team away">
              ...
              {renderCrest(awayTeam.crest || '🇵🇹', awayTeam.name, 'native-scoreboard-crest')}
            </div>
          </div>

          {/* Below: Minute */}
          <div className={`native-scoreboard-status ${matchStatus === 'LIVE' ? 'live' : ''}`}>
            {matchStatus === 'LIVE' 
              ? `دقیقه ${matchMinutes}'` 
              : matchStatus === 'WAITING' 
                ? 'شروع نشده' 
